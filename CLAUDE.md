...
- UI Sizing: UI変更時は主要な高さ・幅を `px` で明示する。あとで詰めやすいように、調整対象の寸法は CSS 変数やまとまった定数へ寄せ、`%` や `auto` だけで主要サイズを決めない。
...
- UI Approval Flow: UI変更時は、ファイル編集前に線や図形を組み合わせたアスキーアートでUIの概形案を提示し、ユーザー承認後にのみ実装へ進むこと。
- UI Approval Flow: 実装後はUI崩れを確認し、変更したパーツの幅・高さ・余白・位置などの主要寸法を `px` で明示すること。
- UI Approval Flow: ユーザーが数値ベースで再修正案を出せるように、調整対象の寸法情報を整理して返し、この承認と再修正のループを前提に進めること。
