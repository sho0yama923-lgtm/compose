...
- GitHub Pages から配信されていることを `Server: GitHub.com` で確認する
...
現在の本番配信先は GitHub Pages。

- GitHub Actions の `Deploy GitHub Pages` workflow が `dist/` を artifact として配信する
- `public/CNAME` に `ezmelon.com` を置き、build 後の `dist/CNAME` に同梱する
- `ezmelon.com` は GitHub Pages の4つの A レコード、`www.ezmelon.com` は `sho0yama923-lgtm.github.io` への CNAME を使う
- GitHub Pages は任意の security header 設定に対応していないため、CSP などは Netlify 時代の `_headers` では本番へ反映されない

## 推奨 Security Headers

将来、Cloudflare Pages や Netlify など header 設定に対応する配信先へ戻す場合は、配信環境の HTTP response header で設定する。
...
Netlify の build credit 切れを避けるため、本番配信は GitHub Pages へ移行済み。旧設定は `legacy/netlify/` に退避している。

- `legacy/netlify/netlify.toml`
- `legacy/netlify/_headers`
